// hw.h : Include file for standard system include files,
...
{
	unsigned char checkheader[PNG_BYTES_TO_CHECK]; //查询是否png头
	*fp = fopen(filename, "rb");
	...
	if (fread(checkheader, 1, PNG_BYTES_TO_CHECK, *fp) != PNG_BYTES_TO_CHECK) //读取png文件长度错误直接退出
		return -1;
	int ans =  png_sig_cmp(checkheader, 0, PNG_BYTES_TO_CHECK); //0正确, 非0错误
	return ans;
}
...
unsigned char* edge_dectetion(unsigned char* src, int srcW, int srcH, int channel, int nthreads=5,int ndivided=9)
{
		...
		for (int i = x * perDesH; i < (x + 1) * perDesH && i < desH; ++i) {
			for (int j = 0; j < desW; ++j) {
				...
				for (int k = 0; k < tchannel; k++) {
					...
					tempSrc[(i * desW + j) * channel + k] = 1.0 / ndivided * ans;
				}
			}
		}
	};
...
int write_png_file(const char* filename, pic_data* out,int nthread=5,int ndivided=9) //生成一个新的png图像
{
	png_structp png_ptr;
	png_infop 	info_ptr;
	png_byte color_type;
	...
	out->rgba = edge_dectetion(out->rgba, out->width, out->height, channels, nthread, ndivided);
	free(ref);
